...
function createBasicAuthMiddleware() {
  ...
  return function basicAuth(req, res, next) {
    if (!BASIC_AUTH_USER || !BASIC_AUTH_PASS) return res.status(401).send('Authentication required');
...
<body>
  ...
  // UI route (optional Basic Auth)
  const basicAuth = createBasicAuthMiddleware();
  const maybeAuth = (req, res, next) => (process.env.BASIC_AUTH_USER && process.env.BASIC_AUTH_PASS ? basicAuth(req, res, next) : next());
  app.get('/', maybeAuth, (req, res) => {
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.send(renderPage());
  });

  // Health: ready when polling store is ready
  ...
  // API: services (Basic auth required)
  const basicAuth = createBasicAuthMiddleware();
  app.get('/api/services', basicAuth, (req, res) => {
    ...
    const payload = {
      ...
      pageTitle: snap.meta && snap.meta.pageTitle,
      pollIntervalMs: snap.meta && snap.meta.pollIntervalMs,
    };
    res.json(payload);
  });

  // Start NPM refresh interval on app creation
  refreshData();
